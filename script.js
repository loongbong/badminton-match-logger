document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!logForm || !historyListDiv || !statsDisplayDiv || !editingMatchIdInput) { console.error("Essential HTML elements missing."); return; }
    ...
    const handleLogMatch = (event) => {
         ...
         const date = matchDateInput.value; const yourPartner = yourPartnerInput.value.trim();
         const opponent1 = opponent1Input.value.trim(); const opponent2 = opponent2Input.value.trim();
         const yourScore = parseInt(yourScoreInput.value); const theirScore = parseInt(theirScoreInput.value);

         // Basic Validation
         if (!date || !opponent1 || isNaN(yourScore) || isNaN(theirScore)) { alert("Fill Date, Opponent 1, and scores."); return; }
         if (yourScore < 0 || theirScore < 0 || yourScore > 30 || theirScore > 30) { alert("Scores must be between 0 and 30."); return; }

         // Badminton Score Logic Validation
         const winnerScore = Math.max(yourScore, theirScore); const loserScore = Math.min(yourScore, theirScore);
         if (winnerScore === loserScore) { alert("Scores cannot be the same."); return; }
         if (winnerScore === 21 && loserScore > 19) { alert("Invalid score: If win is 21, opponent must be <= 19."); return; }
         if (winnerScore > 21 && winnerScore < 30 && winnerScore - loserScore !== 2) { alert(`Invalid score: Win by 2 points required (22-29). Score: ${winnerScore}-${loserScore}.`); return; }
         if (winnerScore === 30 && loserScore < 28) { alert(`Invalid score: If win is 30, opponent must be 28 or 29. Score: 30-${loserScore}.`); return; }

         // Name Conflict Check (only for new entries)
         if (!editingId) { if (yourPartner && (yourPartner === opponent1 || (opponent2 && yourPartner === opponent2))) { const ok = window.confirm(`Warning: Partner "${yourPartner}" also opponent?`); if (!ok) return; } if (opponent1 && opponent2 && opponent1 === opponent2) { const ok = window.confirm(`Warning: Opponents "${opponent1}" same?`); if (!ok) return; } }

         // If Validation Passed
         const win = yourScore > theirScore; const yourTeamName = getCanonicalTeamName("You", yourPartner); const opponentTeamName = getCanonicalTeamName(opponent1, opponent2);
         ...
         saveLogs();
         isShowingAllHistory = false; // Reset history view state on log/update
         updateUI();

         if (editingId) { handleCancelEdit(); } // Reset form after update
         else { // Clear form after adding new (KEEP DATE)
            ...
            // if(opponent1Input) opponent1Input.focus(); // Focus removed based on previous request
        }
    ...
    /** --- NEW: Handler to blur input after datalist selection --- */
    const handleNameInputBlur = (event) => {
        const inputElement = event.target;
        const listId = inputElement.getAttribute('list');
        if (!listId) return; // No datalist associated

        const dataList = document.getElementById(listId);
        if (!dataList) return; // Datalist not found

        const currentValue = inputElement.value;

        // Check if the current value exactly matches an option in the datalist
        ...
        // If an exact match was found (meaning user likely selected an option)
        // then blur the input field to potentially hide mobile keyboard
        if (matchFound) {
             // Add a tiny delay before blurring, sometimes helps mobile browsers
            setTimeout(() => {
                inputElement.blur();
            }, 100);
        }
    ...
    // NEW Listeners for name inputs to handle blur
    if (yourPartnerInput) yourPartnerInput.addEventListener('input', handleNameInputBlur);
    if (opponent1Input) opponent1Input.addEventListener('input', handleNameInputBlur);
    if (opponent2Input) opponent2Input.addEventListener('input', handleNameInputBlur);
